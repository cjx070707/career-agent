...
- 不新增 roadmap 任务行，除非用户明确批准。
- 如果认为必须新增或拆分任务，先说明原任务、新任务、拆分理由、依赖关系和验收标准，并等待用户确认。
- 如果只是完成了原任务的一部分，只更新原任务的状态或验收描述，不新增任务。
- 能力清单可以记录已实现能力；任务清单保持原结构，除非用户批准调整。
- 每次修改本文件的任务清单前，先明确这次是“更新现有任务”还是“请求新增任务”。
...
能力层          app/services/
                  ...
                  ├─ RetrievalService      ChromaDB 向量召回 + BM25 + RRF + rerank + metadata 过滤
...
| BM25 lexical 召回 | ✅ | `retrieval_service._bm25_rank` | #4 |
| RRF 融合（BM25 + 向量） | ✅ | `retrieval_service._rrf_merge` | #4 |
| 关键事件向量索引 | ✅ | `app/services/retrieval_service.py` | #1 |
...
| React 双页面前端 | ✅ | `web/` | #5, #6 |
| VL 图像输入（简历截图解析，Qwen3-VL） | ⚠️ | `app/api/vision.py` `app/llm/vision_client.py` | #5 |
...
| VL 图像输入（简历/JD 截图解析） | React 前端 | #5 | ~5 天 | 简历截图解析完成；JD 截图解析待做 |
...
| #5 图片多模态 + 多端交互 | 50% — React 双页面 ✅；Qwen3-VL 简历截图解析 ✅；JD 截图解析 ❌ | VL 图像输入（补齐 JD） |
